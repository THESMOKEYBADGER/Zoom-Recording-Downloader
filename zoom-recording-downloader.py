...
MEETING_FILENAME = config("Recordings", "filename", '{meeting_time} - {topic} - {file_extension}')
...
def format_filename(params):
    ...
    recording_type = params.get("recording_type", "")

    invalid_chars_pattern = r'[<>:"/\\|?*\x00-\x1F]'
    topic = regex.sub(invalid_chars_pattern, '', recording.get("topic", ""))
    rec_type = recording_type.replace("_", " ").title()
    
    if recording:
        meeting_time_utc = parser.parse(recording.get("start_time", "")).replace(tzinfo=datetime.timezone.utc)
...
def list_recordings(email, start_date, end_date):
    """ Get recordings within the specified start and end dates
    """
    recordings = []
...
def download_recording(download_url, email, filename, folder_name):
    ...
    if not filename.lower().endswith('.mp4'):
        logger.info(f"Skipping non-MP4 file: {filename}")
        return False
    ...
    try:
        with open(full_filename, 'wb') as f:
            # Download the recording in chunks
            for data in response.iter_content(block_size):
                prog_bar.update(len(data))
                f.write(data)
    
    except Exception as e:
        logger.error(f"Unable to download recording for {email} with error: {e}")
        return False
...
def main():
    ...
    from_date_str = input("Enter the start date for recording downloads (YYYY-MM-DD): ")
    
    # Prompt the user for end date
    to_date_str = input("Enter the end date for recording downloads (YYYY-MM-DD): ")

    # Validate dates
    if not validate_dates(from_date_str, to_date_str):
        sys.exit(1)
    ...
    for email, user_id, first_name, last_name in users:
        if first_name and last_name and user_name.lower() in (first_name.lower(), last_name.lower()):
            selected_user = (email, user_id, first_name, last_name)
            break

    if not selected_user:
        logger.error(f"No user found with the name: {user_name}")
        return

    email, user_id, first_name, last_name = selected_user
    userInfo = (
    ...
    # Load completed meeting IDs
    load_completed_meeting_ids()

    # Fetch recordings within the specified date range
    recordings = list_recordings(user_id, from_date, to_date)
    total_count = len(recordings)
    logger.info(f"==> Found {total_count} recordings")

    for index, recording in enumerate(recordings):
        success = False
        meeting_id = recording["uuid"]
        if meeting_id in COMPLETED_MEETING_IDS:
            logger.info(f"==> Skipping already uploaded meeting: {meeting_id}")
            continue
        ...
        for file_type, file_extension, download_url, recording_type, recording_id in downloads:
            if recording_type != 'incomplete':
                ...
                try:
                    ...
                    logger.error(f"Unable to download recording for {userInfo} with error: '{e}'")
                    continue

            else:
                logger.error(f"Incomplete Recording ({index + 1} of {total_count}) for recording with id '{recording_id}'")
                success = False
